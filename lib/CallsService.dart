  ...
  Future<List<Contact>>? fetch_top_cotnact()async{
    try {
      ...
      entries.forEach((element) { 
        ...
        DateTime dateTime = DateTime.fromMillisecondsSinceEpoch(element.timestamp ?? 0);
        if(c0 != null)
        {if(dateTime.isBefore(c0.lastcall)){
           dateTime = c0.lastcall;
        }
        }
       callsmap[element.number ?? ""] = Contact(element.number ?? "",dateTime ,(c0?.totalduration  ??0) + (element.duration ?? 0)); 
